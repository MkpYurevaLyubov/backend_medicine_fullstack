...
const {findUser, userWithToken, deleteRfrTokenInUser} = require('../../Models/db/requests');
...
module.exports.refreshToken = (req, res) => {
  ...
  jwt.verify(refresh_token, process.env.TOKEN_RFR_SECRET, async (err, result) => {
    if (err) {
    ...
    if (refresh_token !== user.refreshToken) return res.status(403).send("Error! Token not correct!");
